...
public class Configuration {
	 ...
	 */
	public Object set(final String path, final Object object,String file_path) {
		checkPath(path);
...
//		System.out.println(  root);
		...
		//更新配置文件
		updateFile(root,file_path);
		return result;
	}
	/***
	 * 将配置设置之后的json值输出到config.json
	 * 注意的是：这个输出的文件路径是写死的
	 * 肖乾柯：note
	 * 时间：2018-04-19
	 */
    //TODO 这里的路径应该与之前的路径统一
	public void updateFile(Object root,String file_path){
		...
		try {
			...
			throw WADataXExcption.WAErrorExcptionInfo(EnumWAErrorCode.CONFIG_FILE_ERROR,
					String.format("配置文件对应路径不存在，该情况是代码编程错误. 请联系开发人员."));
		}
		try {
			...
			throw WADataXExcption.WAErrorExcptionInfo(EnumWAErrorCode.CONFIG_FILE_ERROR,
					String.format("配置文件输出IO异常，请联系开发人员。"));
		}
	 ...
	 */
	public Object remove(final String path,String file_path) {
	...
	public Configuration merge(final Configuration another,
		...
		for (final String key : keys) {
			...
			if (updateWhenConflict) {
				this.set(key, another.get(key),null);
				continue;
			}
